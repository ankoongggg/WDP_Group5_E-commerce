...
export const useSellerRequests = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            const response = await axios.get('http://localhost:9999/api/store/admin/pending-sellers', {
    ...
    const handleApprove = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('accessToken');
            await axios.put(`http://localhost:9999/api/store/admin/approve-seller/${id}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleReject = async (id: string) => {
        ...
        try {
            const token = localStorage.getItem('accessToken');
            await axios.delete(`http://localhost:9999/api/store/admin/reject-seller/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
